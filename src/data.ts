...
export function parseHistoricalData(response: HistoricalDataResponse): HistoricalSector[] {
...
export async function fetchHistoricalData(): Promise<HistoricalSector[]> {
  const config = getConfig();
...
export async function fetchCompanyInfo(ticker: string): Promise<any> {
  const config = getConfig();
